    ...
    for result in results:
        for box, conf, cls in zip(result.boxes.xyxy, result.boxes.conf, result.boxes.cls):
            ...
            if classifier_model is not None and cropped_vehicle.size > 0:
                ...
                label = "Mobil" if prediction > 0.5 else "Bukan Mobil"

            # Gambar bounding box dan label
            color = (0, 255, 0) if label == "Mobil" else (255, 0, 0)
...
def process_video(video_path, yolov8_model, classifier_model, confidence_threshold):
    ...
    stop_button = st.button("Hentikan Pemrosesan Video")
        ...
        if uploaded_file:
            ...
            if st.button("Deteksi Mobil"):
                with st.spinner("Memproses..."):
                    ...
                    # Tampilkan tombol unduh untuk video hasil
